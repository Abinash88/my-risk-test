...
const RepostModal: React.FC<PauseModalProps> = ({
  isModalOpen,
  closeModal,
}) => {
  return (
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-[#D4D9E0] pb-3 pt-3">
          <div className="flex items-center gap-3 ml-3">
            ...
            />
            <p className="text-[#1A1A1A] font-[600]">Repost Risk</p>
          </div>
          <FontAwesomeIcon
            icon={faXmark}
            onClick={closeModal}
        ...
        <p className="py-4 px-5 text-[#666666] leading-[23.2px] text-sm">
          Are you certain you wish to repost this risk? Reposting this risk will
          return it back to “Open Risk” Section and make it visible.
        </p>
        <div className="flex items-center justify-end px-4 gap-3 py-3">
          <button
            className="px-4 py-2 rounded-lg scale-90 border border-[#D9D9D9] text-[#1A1A1A]"
            onClick={closeModal}
          >
            Cancel
          </button>
          <button className="px-4 py-2 scale-90 rounded-lg bg-[#000080] text-white">
            Repost
          </button>
        </div>
      </Modal>
